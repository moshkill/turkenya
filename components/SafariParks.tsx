...
const parks: Park[] = [
  ...
  { name: 'Tsavo East', region: 'Inland · South-East', img: 'https://images.unsplash.com/photo-1521128371335-c9d171809d8f?w=900&q=80&fit=crop', best: 'Jun – Oct',
  ...
  { name: 'Lake Nakuru', region: 'Inland · Rift Valley', img: 'https://images.unsplash.com/photo-1679609071155-4e2a52ee4ff5?w=900&q=80&fit=crop', best: 'Year-round',
  ...
  const Chips = ({ title, items }: { title: string; items: string[] }) => (
    <div style={{ marginBottom: 22 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {items.map(it => (
          <span key={it} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 100, padding: '8px 14px', fontSize: 14, color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>
        ...
        <div className="sb-modal" onClick={() => setOpen(null)} style={{ position: 'fixed', inset: 0, zIndex: 100001, background: 'rgba(5,5,5,0.72)', backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '6vh 16px 16px', overflowY: 'auto' }}>
          <div onClick={e => e.stopPropagation()} style={{ position: 'relative', width: '100%', maxWidth: 640, background: 'rgba(15,15,15,0.98)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 24, overflow: 'hidden', boxShadow: '0 40px 100px rgba(0,0,0,0.6)' }}>
            ...
            <div style={{ padding: '30px 34px 38px' }}>
              <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: 16, lineHeight: 1.7, margin: '0 0 18px' }}>{p.blurb}</p>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.05)', borderRadius: 100, padding: '7px 14px', fontSize: 14, color: 'rgba(255,255,255,0.75)', marginBottom: 24 }}>
              ...
              <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: 15, lineHeight: 1.8, marginBottom: 26 }}>{p.stays.join(' · ')}</div>
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <BookingButton flowKey="safari" label={`Plan ${p.name.split(' ')[0]}`} initial={{ park: p.name }} className="glass-cta" style={{ flex: 1, minWidth: 160, padding: '15px', borderRadius: 100, fontSize: 15, fontWeight: 800, letterSpacing: 1, textTransform: 'uppercase' }} />
