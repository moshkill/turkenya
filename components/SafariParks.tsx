...
const parks: Park[] = [
  ...
  { name: 'Tsavo', region: 'Inland · East & West', img: 'https://images.unsplash.com/photo-1516426122078-c23e76319801?w=900&q=80&fit=crop', best: 'Jun – Oct',
    blurb: 'Vast wilderness of dust-red elephants, Mzima Springs and Lugard Falls.',
    wildlife: ['“Red” elephants & buffalo', 'Lions, leopards, cheetahs', 'Black rhino (Ngulia)', 'Gerenuk, oryx, kudu'],
    activities: ['Long-distance game drives', 'Underwater hippo viewing', 'Shetani lava caves', 'Bird watching (500+)'],
    stays: ['Kilaguni Serena Lodge', 'Finch Hattons', 'Satao Camp', 'Voi Safari Lodge'] },
    ...
    <>
      ...
      {p && (
        <div className="sb-modal" onClick={() => setOpen(null)} style={{ position: 'fixed', inset: 0, zIndex: 100001, background: 'rgba(5,5,5,0.72)', backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '6vh 16px 16px', overflowY: 'auto' }}>
          <div onClick={e => e.stopPropagation()} style={{ position: 'relative', width: '100%', maxWidth: 580, background: 'rgba(15,15,15,0.98)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 24, overflow: 'hidden', boxShadow: '0 40px 100px rgba(0,0,0,0.6)' }}>
