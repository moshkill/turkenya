      ...
      {tiles.map(t => (
        <Link key={t.cls} href={t.href} className={`bento-tile ${t.cls}`}>
          ...
          <div className="bento-body">
            {t.tag && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(255,240,0,0.04)', color: '#fff', border: '1px solid rgba(255,240,0,0.1)', backdropFilter: 'blur(4px)', fontSize: t.big ? 11 : 10, fontWeight: 800, letterSpacing: 1.5, textTransform: 'uppercase', padding: '5px 12px', borderRadius: 100, marginBottom: 14 }}>{t.tag}</span>
            )}
            ...
            {t.cta && <span style={{ marginTop: 18, display: 'inline-block', background: 'rgba(255,240,0,0.04)', border: '1px solid rgba(255,240,0,0.5)', backdropFilter: 'blur(4px)', color: '#fff', padding: '12px 26px', borderRadius: 100, fontSize: 12, fontWeight: 800, letterSpacing: 1.5, textTransform: 'uppercase' }}>{t.cta}</span>}
          </div>
        </Link>
      ))}
    </div>
