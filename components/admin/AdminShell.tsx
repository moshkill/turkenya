...
const pill = (activeColor: boolean): React.CSSProperties => ({
  padding: '7px 14px', borderRadius: 100, fontSize: 14, fontWeight: 700, textDecoration: 'none',
    ...
    <main style={{ minHeight: '100vh', background: '#0a0a0a', fontFamily: "'Abel',sans-serif", color: '#fff' }}>
      <header style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(13,13,13,0.92)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(255,240,0,0.12)', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 60, gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexShrink: 0 }}>
          {me && <span className="desktop-nav" style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)' }}>{me.name.split(' ')[0]} · <span style={{ color: me.role === 'admin' ? '#fff000' : 'rgba(255,255,255,0.55)' }}>{me.role}</span></span>}
          {setAuto && <button onClick={() => setAuto(!auto)} title="Toggle auto-refresh (60s)" style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.65)', padding: '8px 14px', borderRadius: 100, cursor: 'pointer', fontSize: 12, fontWeight: 600 }}><span className={auto ? 'admin-live' : ''} style={{ width: 8, height: 8, borderRadius: '50%', background: auto ? '#22c55e' : '#6b7280', display: 'inline-block' }} />{auto ? 'Live' : 'Paused'}</button>}
          {onRefresh && <button onClick={onRefresh} className="glass-ghost" style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 16px', borderRadius: 100, cursor: 'pointer', fontSize: 14, fontWeight: 600 }}><Icon name="refresh" size={15} /> <span className="desktop-nav">Refresh</span></button>}
          <button onClick={logout} style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'rgba(255,60,60,0.1)', border: '1px solid rgba(255,60,60,0.3)', color: '#ff6b6b', padding: '8px 16px', borderRadius: 100, cursor: 'pointer', fontSize: 14, fontWeight: 600 }}><Icon name="logout" size={15} /> <span className="desktop-nav">Logout</span></button>
        </div>
      </header>
      {children}
    </main>
