...
export default function Header() {
  ...
  return (
    <header style={{
      ...
      borderBottom: scrolled ? '1px solid rgba(255,240,0,0.15)' : 'none',
      transition: 'all 0.3s',
    }}>
      <div style={{
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        height: isMobile ? 68 : 82, padding: isMobile ? '0 20px' : '0 48px',
      }}>
        ...
        <Link href='/' style={{ textDecoration: 'none', flexShrink: 0 }}>
          <img
            src='/logo.png'
            alt='Turkenya Tours & Safaris'
            style={{ height: isMobile ? 46 : 58, width: 'auto', display: 'block' }}
          />
        </Link>
          ...
          <nav style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
            ...
            <div ref={toursRef} style={{ position: 'relative' }}>
              <button
                ...
                style={{ ...ls, background: 'none', border: 'none', cursor: 'pointer', color: toursOpen ? '#fff000' : 'rgba(255,255,255,0.82)', display: 'flex', alignItems: 'center', gap: 4, padding: 0 }}
                onMouseEnter={(e) => hov(e, '#fff000')}
                onMouseLeave={(e) => { if (!toursOpen) hov(e, 'rgba(255,255,255,0.82)') }}
              >
                TOURS ▾
              </button>
              {toursOpen && (
                <div style={{ position: 'absolute', top: 'calc(100% + 14px)', left: '50%', transform: 'translateX(-50%)', background: 'rgba(8,8,8,0.98)', border: '1px solid rgba(255,240,0,0.2)', borderRadius: 8, overflow: 'hidden', minWidth: 180, boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
                  {[{ label: 'Safaris', href: '/safaris' }, { label: 'International', href: '/international' }].map((t) => (
                    <Link
                      key={t.href} href={t.href} onClick={() => setToursOpen(false)}
                      style={{ display: 'block', padding: '14px 22px', color: 'rgba(255,255,255,0.82)', fontSize: 11, fontWeight: 700, letterSpacing: '1.5px', textDecoration: 'none', textTransform: 'uppercase', borderBottom: '1px solid rgba(255,255,255,0.06)', transition: 'all 0.15s' }}
                      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = '#fff000'; (e.currentTarget as HTMLElement).style.background = 'rgba(255,240,0,0.06)' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          <Link
            href='/contact'
            style={{ background: '#fff000', color: '#0D0D00', padding: isMobile ? '9px 16px' : '11px 24px', fontSize: isMobile ? 10 : 11, fontWeight: 800, letterSpacing: '2px', textDecoration: 'none', borderRadius: 3, whiteSpace: 'nowrap' }}
          >
            BOOK NOW
          </Link>
      ...
      {open && (
        <div style={{ background: 'rgba(8,8,8,0.99)', borderTop: '1px solid rgba(255,240,0,0.12)', padding: '8px 24px 28px', maxHeight: '80vh', overflowY: 'auto' }}>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', letterSpacing: '3px', textTransform: 'uppercase', padding: '14px 0 8px' }}>All Services</div>
          {mobileNav.map((n) => (
            <Link
              key={n.href}
              href={n.href}
              ...
              style={{
                display: 'block',
                color: n.href === '/contact' ? '#fff000' : 'rgba(255,255,255,0.88)',
                fontSize: 15,
                fontWeight: 600,
                padding: '12px 0',
                textDecoration: 'none',
