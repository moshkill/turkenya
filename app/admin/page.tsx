...
const CARD: React.CSSProperties = { position: 'relative', background: 'rgba(255,255,255,0.035)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 24, padding: 32, boxShadow: '0 14px 44px rgba(0,0,0,0.28)' }
...
export default function AdminLeadsPage() {
  ...
  // full-width table rows (paginated) — sit in the centre pane; click opens the drawer
    ...
    return (
      <div key={l.id} role="button" tabIndex={0} onClick={() => setSelected(l)} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setSelected(l) } }} className={'lead-row' + (selected?.id === l.id ? ' active' : '')}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
          <span style={{ width: 40, height: 40, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 14, fontFamily: "'Urbanist',sans-serif", color: sc, background: sc + '22', border: '1px solid ' + sc + '44' }}>{initials(l.name)}</span>
    ...
    <AdminShell active="leads" me={me} authed onAuth={() => fetchData()} auto={auto} setAuto={setAuto} onRefresh={fetchData} onLogout={() => { setAuthed(false); setLeads([]); setMe(null) }}>
      <div style={{ maxWidth: 1440, margin: '0 auto', padding: '36px 40px 76px' }}>
        ...
        <div className="admin-shell2">
          ...
          {/* CENTRE — leads table; click a row to open the detail drawer */}
          <section className="glass-card" style={{ padding: 0, overflow: 'hidden' }}>
            ...
            {loading && !total ? <div style={{ padding: 60, textAlign: 'center', color: 'rgba(255,255,255,0.4)' }}>Loading…</div>
              : displayed.length === 0 ? <div style={{ padding: 70, textAlign: 'center', color: 'rgba(255,255,255,0.4)', fontSize: 16 }}>No leads here.</div>
              : rows}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '14px 22px', borderTop: '1px solid rgba(255,255,255,0.07)', flexWrap: 'wrap' }}>
              <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>{displayed.length === 0 ? '0 leads' : `Showing ${pageStart + 1}–${Math.min(pageStart + PAGE_SIZE, displayed.length)} of ${displayed.length}`}{displayed.length !== total ? ` · ${total} total` : ''}</span>
              {pageCount > 1 && (
      ...
      {/* DETAIL DRAWER — slides in from the right when a quote is clicked */}
      {selected && (
        <>
